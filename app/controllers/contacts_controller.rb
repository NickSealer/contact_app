...
# Controller to manage contacts methods
class ContactsController < ApplicationController
  ...
  def index
    @contacts = if params[:status].present? && !params[:status].blank?
                  current_user.contacts.where(is_valid: params[:status]).order(created_at: :desc).paginate(
                    page: params[:page], per_page: 10
                  )
                else
                  current_user.contacts.valid.order(created_at: :desc).paginate(page: params[:page], per_page: 10)
                end
  end
  ...
  def download_csv
    layout = CSV.generate do |csv|
      csv << %w[name birthdate phone address credit_card email]
    end
